...
export interface Asset extends NodeTrivia {
  id: string; kind: "image" | "font" | "iconpack"; src: string; meta?: any
}

export interface Component extends NodeTrivia {
  id: string; name: string; nodes: Node[]; props?: Record<string, PropSpec>;
}

export interface PropSpec { type: "string" | "number" | "boolean"; default?: any }

export interface Page extends NodeTrivia { id: string; name: string; frames: Frame[] }
...
export interface TextStyle {
  font?: string; size?: number; weight?: number;
  align?: "left" | "center" | "right"; wrap?: boolean; autoSize?: boolean;
}
...
function parsePage(C: ParseContext, indent: number, page: Page) {
  while (peek(C)) {
    ...
    const m = tok.text.match(/^frame\s+([^:]+):\s*$/);
    if (!m) { C.errors.push({ line: tok.lineNo, column: 1, message: "Expected 'frame Id:'" }); C.i++; continue; }

    C.i++;
    const frame: Frame = { kind: "frame", id: slugify(m[1]), name: m[1].trim(), w: 0, h: 0, children: [] } as any;
    attachComments(frame, C);
    parseNodeBlock(C, indent + 1, frame, true);
